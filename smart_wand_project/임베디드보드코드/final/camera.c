...
{ 
	...
	reverse = 160;
	for (y = videoHeight - 1; y >= 160; y--)
	{
		for (x = videoWidth - 1; x >= 320; x--)
		{
 
			videptrTemp = videoptr + videoWidth*y + x;
 		        image[(reverse-160)*640 + (x-320)].r = (unsigned char)((*videptrTemp & 0xF800) >> 8);
 		        image[(reverse-160)*640 + (x-320)].g = (unsigned char)((*videptrTemp & 0x07E0) >> 3);
 		        image[(reverse-160)*640 + (x-320)].b = (unsigned char)((*videptrTemp & 0x001F) << 3);
          		}
			reverse++;
	}
}
...
void make_bmp(char *name)
{
	...
	printf("%d%d, %d, %d, %d, %d\n",magicNum[0],magicNum[1], bmp_head.bfSize, bmp_head.bfReserved1, bmp_head.bfReserved2, bmp_head.bfOffBits);
	
	bmp_info.headerSize = 40;
	bmp_info.biWidth = 640;
	bmp_info.biHeight = 480;
	bmp_info.biPlanes = 1;
	...
	if (!(fp = fopen(name, "wb")))
		return;
	fwrite(magicNum, 2, sizeof(unsigned char), fp);
...
void putPicture(char* bmp)
{
	...
	servAddr.sin_addr.s_addr = inet_addr("13.124.115.165");
	servAddr.sin_port = htons((unsigned short)3000);
	if(connect(sock, (struct sockaddr*)&servAddr, servAddrLen))
	{
	...
	timer = time(NULL);
	t = localtime(&timer);
	bufferLen = fread(buffer + 22, 1, 1048554, fbmp) + 22;
	printf("h111 \n");
	sprintf(buffer, "%04d-%02d-%02d+%02d-%02d-%02d%03d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
							t->tm_hour, t->tm_min, t->tm_sec,
							(bufferLen / 1000) + (bufferLen % 1000 ? 1 : 0));
	
	printf("picture name : %s\n",buffer);
	int i;
	buffer[22] = 'B';
	for(i = 0; i < (bufferLen / 1000); i++)
	{	printf("here \n");
		if(send(sock, buffer + i * 1000, 1000, 0) != 1000)
		{
			printf("send error %d\n", i);
			fclose(fbmp);
			close(sock);
	...
	if(bufferLen % 1000)
	{	printf("here2 \n");
		if(send(sock, buffer + (bufferLen / 1000) * 1000, bufferLen % 1000, 0) != bufferLen % 1000)
		{
			printf("send error else\n");
			fclose(fbmp);
			close(sock);
			return;
		}

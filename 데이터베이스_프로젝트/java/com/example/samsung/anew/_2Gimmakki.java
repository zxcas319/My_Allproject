...
public class _2Gimmakki extends Info {
    String Re;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity__2_gimmakki);
 ...
 **/
        Cursor cursor = db.rawQuery("select distinct Name from Food, Course where((Fnumber = SetMenu AND Fname = 8) OR (Fnumber = Fname AND SetMenu = 8))", null);
         ...
         **/
        cursor = db.rawQuery("select Recipe from Recipe where(id == 52)", null);
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 53)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 54)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 55)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 56)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
     ...
     **/
    public void gomenuButtonClicked2(View v) {
     ...
     */
    public void gobackButtonClicked2(View v) {
        finish();
    }
}
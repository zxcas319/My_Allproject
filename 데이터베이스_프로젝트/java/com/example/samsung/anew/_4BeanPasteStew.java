...
public class _4BeanPasteStew extends Info {
    String Re;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity__4_bean_paste_stew);
 ...
 **/
        Cursor cursor = db.rawQuery("select distinct Name from Food, Course where((Fnumber = SetMenu AND Fname = 1) OR (Fnumber = Fname AND SetMenu = 1))", null);
 ...
 **/
        cursor = db.rawQuery("select Recipe from Recipe where(id == 1)", null);
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 2)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 3)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 4)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 5)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 6)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
     ...
     **/
    public void gomenuButtonClicked4(View v) {
     ...
     */
    public void gobackButtonClicked4(View v) {
        finish();
    }
}
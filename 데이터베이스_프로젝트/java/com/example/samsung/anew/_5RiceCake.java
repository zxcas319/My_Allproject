...
public class _5RiceCake extends Info {
    String Re;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity__5_rice_cake);
 ...
 **/
        Cursor cursor = db.rawQuery("select distinct Name from Food, Course where((Fnumber = SetMenu AND Fname = 12) OR (Fnumber = Fname AND SetMenu = 12))", null);
 ...
 **/
        cursor = db.rawQuery("select Recipe from Recipe where(id == 75)", null);
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 76)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 77)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 78)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 79)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 80)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 81)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
     ...
     **/
    public void gomenuButtonClicked5(View v) {
     ...
     */
    public void gobackButtonClicked5(View v) {
        finish();
    }
}
...
public class _6Ddukbulgogi extends Info {
    String Re;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity__6_ddukbulgogi);
 ...
 **/
        Cursor cursor = db.rawQuery("select distinct Name from Food, Course where((Fnumber = SetMenu AND Fname = 2) OR (Fnumber = Fname AND SetMenu = 2))", null);
 ...
 **/
        cursor = db.rawQuery("select Recipe from Recipe where(id == 7)", null);
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 8)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 9)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 10)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 11)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
     ...
     **/
    public void gomenuButtonClicked6(View v) {
     ...
     */
    public void gobackButtonClicked6(View v) {
        finish();
    }
}
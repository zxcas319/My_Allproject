...
public class _7FruitJuice extends Info {
    String Re;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity__7_fruit_juice);
 ...
 **/
        Cursor cursor = db.rawQuery("select distinct Name from Food, Course where((Fnumber = SetMenu AND Fname = 10) OR (Fnumber = Fname AND SetMenu = 10))", null);
 ...
 **/
        cursor = db.rawQuery("select Recipe from Recipe where(id == 66)", null);
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 67)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        cursor = db.rawQuery("select Recipe from Recipe where(id == 68)", null);
        count = cursor.getCount();
        for (int i = 0; i < count; i++) {
            cursor.moveToNext();
        ...
        ListView list = (ListView) findViewById(R.id.List7);
     ...
     **/
    public void gomenuButtonClicked7(View v) {
     ...
     */
    public void gobackButtonClicked7(View v) {
        finish();
    }
}